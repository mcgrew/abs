...
REPO_NAME="mcgrew"
REPO_DIR="/var/www/html/archlinux/$REPO_NAME/os/x86_64"
...
UPDATE_REPO="/usr/bin/repo-add ${REPO_DIR}/${DB_FILE}.tar.gz ${REPO_DIR}/*.pkg.tar.*"
...
if [ ! -z "$REPO_DIR" ]; then
	...
	if [ ! -z "${DB_FILE}" ]; then
		if [ "${newest:0:${#DB_FILE}}" != "${DB_FILE}" ]; then
			...
			echo "Repo database already up to date."
		fi;
	else
